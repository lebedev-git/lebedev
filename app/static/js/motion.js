...
// ── Чужой service worker ─────────────────────────────────────────────────────
// На домене раньше жило другое приложение с PWA; его воркер в браузерах
// посетителей снимаем сразу, не дожидаясь проверки обновления (см. /sw.js).
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.getRegistrations()
    .then((regs) => Promise.all(regs.map((r) => r.unregister())))
    .then(() => 'caches' in window && caches.keys().then((ks) => ks.forEach((k) => caches.delete(k))))
    .catch(() => {});
}
